...
export class PanelDesignerComponent implements AfterViewInit {
  ...
  readonly STUB_LEN = 12;            // initial vertical stub length from connector (px)
...
private verticallyAligned(a: PlacedPart, b: PlacedPart): boolean {
  ...
  onClickConnector(partId: string, port: Port) {
    ...
    const conn: Connection = { id: crypto.randomUUID(), from, to: here, color };
  ...
  /** Returns the polyline points string for SVG (includes initial vertical stubs) */
getPolylinePoints(conn: Connection): string {
  const a = this.connectorCenter(conn.from);
  const b = this.connectorCenter(conn.to);
  if (!a || !b) return ''; // invisible

  // --- Initial snapped stubs so first segment is vertical and grid-aligned ---
...
// --- NEW: fan-out lanes at each port (horizontal jog after the vertical stub) ---
const laneOffset = (ref: ConnectorRef) => {
  const { index, total } = this.portSlot(ref, conn.id);
  if (total <= 1) return 0;
  const center = (total - 1) / 2;                // center lanes around 0
  const raw = (index - center) * this.FANOUT_GAP; // e.g. -10, 0, +10, ...
  return snap(raw);                               // keep aligned to grid
};

const fromDx = laneOffset(conn.from);
const toDx   = laneOffset(conn.to);

// Anchors that the router connects between (purely horizontal fan-out from stub level)
const aAnchor = { x: snap(aStub.x + fromDx), y: aStub.y };
const bAnchor = { x: snap(bStub.x + toDx),   y: bStub.y };

  const width  = this.panelRef.nativeElement.clientWidth || (this.getRailLeft() + this.RAIL_WIDTH * 2);
  const height = this.panelHeight();

  // --- Component obstacles (inflated) ---
  const partRects = this.parts().map(p => this.inflate(this.partRectAbs(p), this.OBSTACLE_CLEAR));

  // --- Rail obstacles: block each rail so router must go around LEFT/RIGHT sides ---
  // Direct vertical exception:
  // If parts are on different rails, exactly aligned, and the ports are
  //   LOWER.top  -> UPPER.bottom  (in either from/to order),
  // then we ALLOW crossing rails vertically (no rail blocks between them).
  ...
  const allowDirectVertical =
    fromPart.railIndex !== toPart.railIndex &&
  ...
  for (const y of this.railsTop()) {
    // If we allow direct vertical, skip adding rail blocks that lie between the stubs
    if (allowDirectVertical) {
      const minY = Math.min(aStub.y, bStub.y);
      const maxY = Math.max(aStub.y, bStub.y);
      if (y >= minY && y <= maxY) {
        continue; // do not block this rail; vertical can pass through here
      }
    }
    railsRects.push({
      x: railLeft,
  ...
  const obstacles = [...partRects, ...railsRects];

  // Route between stubs (stubs already move line just outside the component)
  //const routed = this.routeOrthogonal(aStub, bStub, obstacles, { w: width, h: height }, this.ROUTE_STEP);
  const routed = this.routeOrthogonal(aAnchor, bAnchor, obstacles, { w: width, h: height }, this.ROUTE_STEP);
  // Connector center -> stub -> routed -> stub -> connector center
  //const pts = [aGrid, aStub, ...routed, bStub, bGrid];
// Compose final polyline: center -> stub -> fan-out -> routed -> fan-in -> stub -> center
const pts = [aGrid, aStub, aAnchor, ...routed, bAnchor, bStub, bGrid];
  // Simplify collinear
  ...
  private routeOrthogonal(start: Pt, goal: Pt, obstacles: Rect[], bounds: { w: number; h: number }, step: number): Pt[] {
    ...
    while (open.length) {
      ...
      if (current === key(G.cx, G.cy)) {
        ...
        pathCells.reverse();
        // Convert to pixel points (center of cells)
...
//   // --- Determine stub directions (up = -1, down = +1) for start/end ---
...
//   //   y: conn.to.port === 'top' ? snap(bGrid.y - stubLen) : snap(bGrid.y + stubLen),
//   // };
// const aStub = { x: aGrid.x, y: snap(aGrid.y + (conn.from.port === 'top' ? -stubLen : +stubLen)) };
//   const bStub = { x: bGrid.x, y: snap(bGrid.y + (conn.to.port   === 'top' ? -stubLen : +stubLen)) };
...
// //   return snap(raw);                               // keep aligned to grid
// // };
// // --- fan-out lanes ONLY at destination (horizontal jog after vertical stub) ---
// const laneOffset = (ref: ConnectorRef) => {
//   const { index, total } = this.portSlot(ref, conn.id);
//   if (total <= 1) return 0;
...
// //const fromDx = laneOffset(conn.from);
// //const toDx   = laneOffset(conn.to);
// const fromDxSrc = this.sourceLaneOffset(conn.from);
// const aAnchor = { x: snap(aStub.x + fromDxSrc), y: aStub.y };
...
//  const toDx = (() => {
//     const { index, total } = this.portSlot(conn.to, conn.id);
//     if (total <= 1) return 0;
//     const center = (total - 1) / 2;
//     return snap((index - center) * this.FANOUT_GAP);
//   })();
...
// // const bAnchor = { x: snap(bStub.x + toDx),   y: bStub.y };
// // i.e., the cell toward the component, so the router can't step back and make a tiny loop.

// //  const cellRect = (cx: number, cy: number): Rect => {
// //   const s = this.ROUTE_STEP;
// //   return { x: cx - s / 2, y: cy - s / 2, w: s, h: s };
// // };
//  const cellRect = (cx: number, cy: number): Rect => ({ x: cx - step/2, y: cy - step/2, w: step, h: step });
...
// // ---------- Anti-tangle guard ONLY at destination ----------
// const endStubDir = conn.to.port === 'top' ? -1 : +1;
// //OPTIONAL ADD
// //const startGuard = cellRect(aAnchor.x, aAnchor.y - startStubDir * this.ROUTE_STEP * 1);
// //const endGuard   = cellRect(bAnchor.x, bAnchor.y - endStubDir   * this.ROUTE_STEP * 1);
// //const endGuard = cellRect(bAnchor.x, bAnchor.y - endStubDir * this.ROUTE_STEP);

// const startDir = conn.from.port === 'top' ? -1 : +1;
//   const endDir   = conn.to.port   === 'top' ? -1 : +1;
//   const startGuard = cellRect(aAnchor.x, aAnchor.y - startDir * step);
//   const endGuard   = cellRect(bAnchor.x, bAnchor.y - endDir   * step);

// const wirePad = Math.max(6, step);
//   const foreignWireRects: Rect[] = [];
//   for (const [id, path] of this.pathCache) {
//     const other = this.connections().find(c => c.id === id);
//     if (!other) continue;
//     if (other.from.partId === conn.from.partId) continue; // allow same source to overlap
//     foreignWireRects.push(...this.wireObstaclesFromPath(path, wirePad));
//   }
//   const width  = this.panelRef.nativeElement.clientWidth || (this.getRailLeft() + this.RAIL_WIDTH * 2);
//   const height = this.panelHeight();
...
// // Decide preferred side only when crossing rails and NOT allowed to go vertical
...
//     // Block RIGHT corridor in this Y band so router must use LEFT outside
//     sideWalls.push({ x: railRight, y: minY, w: Math.max(1, (this.panelRef.nativeElement.clientWidth - railRight)), h: maxY - minY });
//   } else {
//     // Block LEFT corridor in this Y band so router must use RIGHT outside
...
// // Existing obstacles you already compute:
// //const obstacles = [...partRects, ...railsRects, ...sideWalls, endGuard];
...
// //   aAnchor ?? aStub,
// //   bAnchor ?? bStub,
// //   obstacles, { w: width, h: height }, this.ROUTE_STEP
// // );

// // Route between the (possibly offset) anchors
// // const routed = this.routeOrthogonal(
// //   aAnchor,
// //   bAnchor,
// //   obstacles,
...
// //   this.ROUTE_STEP
// // );
//  const routed = this.routeOrthogonal(aAnchor, bAnchor, obstacles, { w: width, h: height }, step);
// const entry = step; // 1 grid unit
//   const bNear = {
//     x: bAnchor.x,
//     y: conn.to.port === 'top' ? snap(bGrid.y + entry) : snap(bGrid.y - entry),
//   };
//   const bLead = { x: bGrid.x, y: bNear.y };

// // --- NEW: tiny destination “entry nub” to keep a visible gap at the port ---
// // This creates a short vertical at offset X, then a short horizontal into center,
// // then the last tiny vertical into the connector center. All 90°.
// // const entryNub = this.ROUTE_STEP; // small, grid-aligned distance (e.g., 10px)
// // const bNear = {
// //   x: bStubOff.x,
...
// // Build final polyline (keep your existing order; if you have fan-out, include it)
// // const pts = aAnchor
// //   ? [aGrid, aStub, aAnchor, ...routed, bAnchor, bStub, bGrid]
// //   : [aGrid, aStub, ...routed, bStub, bGrid];
// // Final points: center → stub → (route) → offset approach → near → short-in → center
// // const pts = [
// //   aGrid,
// //   aStub,
// //   ...routed,
// //   // already at bAnchor (= bStubOff)
// //   // ensure a vertical leg down/up from bAnchor to bNear (if needed)
// //   bNear,
// //   bLead,
// //   bGrid
// // ];
//   const pts = [aGrid, aStub, aAnchor, ...routed, bAnchor, bNear, bLead, bGrid];
//   const simplified = this.simplifyOrthogonal(pts.map(p => ({ x: snap(p.x), y: snap(p.y) })));
//  // const simplified = this.simplifyOrthogonal(pts);
//   this.pathCache.set(conn.id, simplified);
//   //return simplified.map(p => `${Math.round(p.x)},${Math.round(p.y)}`).join(' ');
//   return simplified.map(p => `${p.x},${p.y}`).join(' ');
