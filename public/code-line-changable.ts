...
onSelectConnection(conn: Connection, ev: MouseEvent) {
  ...
  // if not manual already, lock in the current auto path
  if (!conn.manual) {
    const currentPath = this.pathCache.get(conn.id);
    if (currentPath && currentPath.length >= 2) {
      conn.manual = true;
      conn.manualPoints = [...currentPath];
    }
  }
...
onWireMouseDown(conn: Connection, ev: MouseEvent) {
  ...
  // If not in manual mode yet, convert current auto path to manual path
  if (!conn.manual) {
    const auto = this.pathCache.get(conn.id);
    if (auto && auto.length >= 2) {
      conn.manual = true;
      conn.manualPoints = [...auto];
    }
  }

  // Ensure manual mode is active
  if (!conn.manual || !conn.manualPoints) return;
  ...
  // Original manual path before drag starts
  const original = conn.manualPoints.map(p => ({ ...p }));

  const move = (e: MouseEvent) => {
    const dx = e.clientX - startX;
    const dy = e.clientY - startY;

    // Entire-wire drag (S3: NO snapping)
    conn.manualPoints = original.map(p => ({
      x: p.x + dx,
      y: p.y + dy
    }));

    // Force redraw using signal update
